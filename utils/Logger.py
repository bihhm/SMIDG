...
default_output_dir = '/DG/running_log/PACS'
...
class Logger():
    def __init__(self, args, config, update_frequency=10):
        ...
        self.log_txt = os.path.join(self.log_path, "log.txt")
        if args.tf_logger:
    ...
    def new_epoch(self, learning_rates):
        ...
        print("New epoch - lr: %s" % ", ".join([str(lr) for lr in self.lrs]), file = open(self.log_txt, "a"), flush = True)
        self._clean_epoch_stats()
        if self.tf_logger:
    ...
    def log(self, it, iters, losses, samples_right, total_samples):
        ...
        if it % self.update_f == 0:
            ...
            print("[%d/%d iteration of epoch %d/%d] \n {loss} %s \n {acc} %s" %
                  (it+1, iters, self.current_epoch, self.max_epochs, loss_string, acc_string), file = open(self.log_txt, "a"), flush = True)
            # update tf log
            if self.tf_logger:
                for k, v in losses.items():
        ...
        if (it+1) % iters == 0:
            ...
            print('-' * 30)
            print('-' * 30, file = open(self.log_txt, "a"), flush = True)
            print("<Losses on train> " + epoch_loss_string)
            print("<Losses on train> " + epoch_loss_string, file = open(self.log_txt, "a"), flush = True)
            print("<Accuracies on train> " + epoch_acc_string)
            print("<Accuracies on train> " + epoch_acc_string, file = open(self.log_txt, "a"), flush = True)
            print("Train epoch time: %.3f" % (time() - self.last_update))
            print("Train epoch time: %.3f" % (time() - self.last_update), file = open(self.log_txt, "a"), flush = True)
            if self.tf_logger:
    ...
    def log_test(self, phase, accuracies):
        ...
        print("<Accuracies on %s> " % phase + ", ".join(["%s : %.2f" % (k, v * 100) for k, v in accuracies.items()]), file = open(self.log_txt, "a"), flush = True)
        if self.tf_logger:
    ...
    def finish_epoch(self):
        ...
        print("Total epoch time: %.3f" % (time() - self.last_update), file = open(self.log_txt, "a"), flush = True)
    ...
    def save_best_acc(self, val_res=None, test_res=None, val_best=None, idx_val_best=None, name='best_acc.json'):
        ...
        print("Best val %g, corresponding test %g - best test: %g" % (val_best, test_res[idx_val_best], test_res.max()), file = open(self.log_txt, "a"), flush = True)
        best_acc_dict = {
    ...
    def save_best_model(self, encoder, classifier, val_acc, name='best_model',epoch=0):
        torch.save({
            'epoch': self.current_epoch,
            'val_acc': val_acc,
            'encoder_state_dict': encoder.state_dict(),
            'classifier_state_dict': classifier.state_dict()
        }, os.path.join(self.log_path, f'{name}-{epoch}.tar'))
